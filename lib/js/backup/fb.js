  ...
  function statusChangeCallback(response) {
    ...
    if (response.status === 'connected') {
      ...
      document.getElementById('status').innerHTML = 'Please log ' +
        'into Facebook' ;
      document.getElementById('email').value = 'fleminganalytic@gmail.com' ;
  ...
  function testAPI() {
    ...
    FB.api('/me', function(response) {
      ...
      document.getElementById('status').innerHTML =
        'Thank you for logging in, ' + response.name ;
        document.getElementById('emailid').innerHTML =
        response.email;
       _user_name=response.name;
        _user_email=response.email;
       $.get(_model_header+"action=get_user&email="+_user_email, function(data)
       {
